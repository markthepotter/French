...
def update_word():
    ...
    for i, button in enumerate(option_buttons):
        button.config(text=options[i], bg="lightgray", command=lambda opt=options[i]: check_answer(opt, answer))
...
difficulty_slider = tk.Scale(root, from_=1, to=10, orient=tk.HORIZONTAL, variable=difficulty, length=600, tickinterval=1)
